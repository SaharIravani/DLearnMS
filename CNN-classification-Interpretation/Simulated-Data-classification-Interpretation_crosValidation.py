...
model_for_interpretation ='model_cross_validation.h5'
###################################
#
###################################
...
def __test__():
    ...
    mean_test_22= np.zeros((image_h, image_w),np.float)
    mean_test_20 = np.zeros((image_h, image_w),np.float)
...
for i in range(200):
    test_image_dif_importance [ indices_diff_image[i][0]-w:indices_diff_image[i][0]+w, \
    indices_diff_image[i][1]-h:indices_diff_image[i][1]+h ] \
        = data[indices_diff_image[i][0]-w:indices_diff_image[i][0]+w, \
          indices_diff_image[i][1]-h:indices_diff_image[i][1]+h]
...
for train, val in kfold.split(fileList, y):
    ...
    def __a__():
        ...
        for i in range(10000):
            ...
            if (ind[0]>w and ind[1]>h and interpretation_mean_test_20[ind[0], ind[1]] >= 0):
        ...
        try:
            ...
            for i in range(200):
                if (indices_[i][0]>w and indices_[i][1]>3):
                    Interpretation_importance [ indices_[i][0]-w:indices_[i][0]+w, indices_[i][1]-h:indices_[i][1]+h] = \
                        data[indices_[i][0]-w:indices_[i][0]+w, indices_[i][1]-h:indices_[i][1]+h]
                else:
        ...
        roi , r_, r__, y_pred_22, y_true_22 = ROI_function(test_image_dif_importance,0.1,Interpretation_importance,0.1)
    fold_no = fold_no + 1
...
def __O__():
        y_reverse_categorical = np.argmax(y, axis=1)
        num_val_22pep = np.count_nonzero(y_reverse_categorical[val]==0)
        num_val_20pep = np.count_nonzero(y_reverse_categorical[val])
        y_val_reverse_categorical =y_reverse_categorical[val]
        for N, file in enumerate(fileList[val]):
            image = io.imread(file)
            #image = skimage.transform.resize(image, (image_h, image_w))
            image = util.invert(image)
            image = image.astype('float')/255
            #image = rgb2gray(image)
            if y_val_reverse_categorical[N] ==1:
                mean_val_20 = mean_val_20 + np.abs(image)/num_val_20pep
            else:
                mean_val_22 = mean_val_22 + np.abs(image)/num_val_22pep
        io.imsave('fold'+str(fold_no)+'_mean_val_20.png', mean_val_20)
        io.imsave('fold'+str(fold_no)+'_mean_val_22.png', mean_val_22)
        model_name = 'fold'+ str(fold_no)+'_model.h5'
        model = load_model(model_name)
        method = "lrp.z"
        model_wo_softmax = iutils.keras.graph.model_wo_softmax(model)
        analyzer = innvestigate.create_analyzer(method, model_wo_softmax)
        mean_val_20_ = mean_val_20[:,:,np.newaxis]
        mean_val_20_ = mean_val_20_[None,:,:,:]
        interpretation_mean_val_20 = analyzer.analyze(mean_val_20_)
        interpretation_mean_val_20 = interpretation_mean_val_20.sum(axis=np.argmax(np.asarray(interpretation_mean_val_20.shape) == 3))
        interpretation_mean_val_20/= np.max(np.abs(interpretation_mean_val_20))
        outputname = str(fold_no) + str(method) +'interpretation_mean_val_20.npy'
        np.save(outputname, interpretation_mean_val_20[:,:,0])
        mean_val_22_ = mean_val_22[:,:,np.newaxis]
        mean_val_22_ = mean_val_22_[None,:,:,:]
        interpretation_mean_val_22 = analyzer.analyze(mean_val_22_)
        interpretation_mean_val_22 = interpretation_mean_val_22.sum(axis=np.argmax(np.asarray(interpretation_mean_val_22.shape) == 3))
        interpretation_mean_val_22/= np.max(np.abs(interpretation_mean_val_22))
        outputname = str(fold_no) + str(method) +'interpretation_mean_val_22.npy'
        np.save(outputname, interpretation_mean_val_22[:,:,0])
        interpretation_mean_val_22 = interpretation_mean_val_22[:,:,0]
        interpretation_mean_val_20 = interpretation_mean_val_20[:,:,0]
        print('fold '+ str(fold_no)+' is completed.')
        fold_no = fold_no+1
